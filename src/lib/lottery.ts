	...
	for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
		try {
			...
			if (attempt < MAX_RETRIES) {
				...
				console.warn(
					`[lottery] fetch attempt ${attempt + 1}/${MAX_RETRIES + 1} failed for ${url}, retrying in ${delay}ms`,
				);
				await sleep(delay);
			}
		}
...
function normalizeContestData(data: ContestData): ContestData {
	return {
		...
		listaRateioPremio: Array.isArray(data.listaRateioPremio)
			? data.listaRateioPremio
			: [],
		listaDezenas: Array.isArray(data.listaDezenas)
			? data.listaDezenas.map((d) => String(d).padStart(2, "0"))
			: [],
...
function validateContestDataOrThrow(data: ContestData): ContestData {
	...
	if (!Array.isArray(data.listaDezenas) || data.listaDezenas.length !== 6)
		throw new Error("invalid payload: dezenas missing/invalid");
...
function buildCaixaWorkerUrl(contestNumber: string): string {
	...
	return contestNumber === ""
		? `${base}/megasena`
		: `${base}/megasena/${contestNumber}`;
}

async function fetchFromCaixaWorker(
	...
	try {
		const data = validateContestDataOrThrow(
			await fetchWithRetry<ContestData>(url, "json"),
		);
		if (contestNumber !== "" && data.numero !== Number(contestNumber)) {
			throw new Error(
				`contest mismatch expected=${contestNumber} got=${data.numero}`,
			);
		}
		return data;
	} catch (error) {
		console.warn(
			`[lottery] fail source=caixa-worker contest=${contestNumber || "latest"} reason=${getErrorMessage(error)}`,
		);
		throw error;
	}
}
...
export async function fetchContestFromApi(
	contestNumber = "",
): Promise<ContestData> {
...
export async function fetchContestData(
	...
	if (contestNumber !== "") {
		...
		if (cached) {
			...
			if (updatedAt) {
				...
				const dateStr = updatedAt.includes("T")
					? updatedAt
					: updatedAt.replace(" ", "T") + "Z";
				const ageMs = Date.now() - new Date(dateStr).getTime();
				if (ageMs > STALE_THRESHOLD_MS) {
					console.log(
						`[lottery] ⚠️ cache stale contest=${contestNumber} age=${Math.round(ageMs / 3600000)}h`,
					);
					try {
						const freshData = await fetchContestFromApi(contestNumber);
						try {
							saveContest(freshData);
						} catch (e) {
							console.error("[lottery] Save fail:", e);
						}
						console.log(
							`[lottery] contest=${contestNumber} source=api strategy=db-first-stale-refresh`,
						);
						return freshData;
					} catch (apiError) {
						console.warn(
							`[lottery] stale refresh failed, returning stale cache contest=${contestNumber} error=${getErrorMessage(apiError)}`,
						);
						console.log(
							`[lottery] contest=${contestNumber} source=db strategy=db-first-stale-fallback`,
						);
						return cached;
					}
				}
			...
			console.log(
				`[lottery] contest=${contestNumber} source=db strategy=db-first`,
			);
			return cached;
		}
	}
	...
	try {
		...
		console.log(
			`[lottery] contest=${contestNumber || "latest"} source=api strategy=${contestNumber !== "" ? "db-first" : "api-first"}`,
		);
		return data;
	} catch (error) {
		if (contestNumber === "") {
