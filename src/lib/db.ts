...
function getDb(): Database | null {
    ...
    `);
		...
		if (betsExists) {
			const cols = db.prepare("PRAGMA table_info(bets)").all() as Array<{
				name: string;
			}>;
        ...
        `);
				...
				if (oldRow) {
					...
					const one_off = JSON.parse(oldRow.one_off) as Record<
						string,
						string[][]
					>;
...
export function getContestCacheAge(contestNumber: number): string | null {
	...
	try {
		const row = database
			.prepare("SELECT updated_at FROM contests WHERE numero = ?")
...
export function getBets(): BetsConfig | null {
	...
	try {
		const rows = database
			.prepare("SELECT numbers, type, contest FROM bets ORDER BY id")
			.all() as Array<{
			numbers: string;
			type: string;
			contest: number | null;
